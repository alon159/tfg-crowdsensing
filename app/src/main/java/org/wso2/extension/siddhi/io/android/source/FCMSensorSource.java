...
@Extension(
        ...
        parameters = {
                @Parameter(
                        name = "type",
                        description = "Simulated sensor type to listen to",
                        type = {DataType.STRING}
                )
        },
        examples = {
                @Example(
                        syntax = "@source(type = 'android-sim-sensor',type = ‘pressure’" +
                                " ,@map(type='keyvalue'))\n" +
                                "define stream fooStream(from String, timestamp long)",
                        description = "”.\n"
                )
        }
)
public class FCMSensorSource extends Source {
    ...
    private class DataUpdateReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            //Log.i("SiddhiSource", "Se ha recibido el evento con las claves"+ bundle.get("hum"));
            ...
            Log.i("SiddhiSource", "envio al flujo: "+ bundle.get("type"));
        }
    }
}
